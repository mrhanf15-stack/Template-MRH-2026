...
if (
    ...
    || !$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_categories2.html', $cache_id)
) {
  ...
  if (MRH_ADD_LINK_IN_TOPCATMENU_LAST != '') {
    ...
    foreach ($add_links as $add_link) {
      ...
      if (substr($links[0], 0, 4) != 'http') {
        $url_parts = (!empty($links[0]) ? explode('?', $links[0]) : array($links[0]));
        $links[0] = xtc_href_link($url_parts[0], (!empty($url_parts[1]) ? $url_parts[1] : ''));
      }					
      $link_content[] = array (
        'HREF' => $links[0],
        'NAME' => $links[1],
