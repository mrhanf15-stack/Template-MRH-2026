...
// blog
if (defined('MODULE_BLOG_STATUS') 
	&& MODULE_BLOG_STATUS == 'true'
	&& !empty($admin_access['blog'])
) {
	$box_smarty->assign('LINK_BLOG', xtc_href_link_admin((defined('DIR_ADMIN') ? DIR_ADMIN : 'admin/').'blog.php', '', 'NONSSL'));
	if (basename($PHP_SELF) == FILENAME_BLOG) {
		if (!empty($_GET['p'])) $box_smarty->assign('EDIT_BLOG_POST', xtc_href_link_admin((defined('DIR_ADMIN') ? DIR_ADMIN : 'admin/').'blog.php', 'action=edit_post&pID='.(int)$_GET['p'], 'NONSSL'));
		if (!empty($_GET['c'])) $box_smarty->assign('EDIT_BLOG_CATEGORY', xtc_href_link_admin((defined('DIR_ADMIN') ? DIR_ADMIN : 'admin/').'blog.php', 'action=edit_category&cID='.(int)$_GET['c'], 'NONSSL'));
	}
}
    ...
    function initColorpickers() {
        ...
        inputs.forEach(function(textInput) {
            ...
            colorInput.addEventListener("input", function() {
                ...
                var cpName = textInput.getAttribute("name");
                if (cpName) mrhSetLiveVar(cpName, rgbVal);
            });
            ...
            textInput.addEventListener("input", function() {
                ...
                var cpName = textInput.getAttribute("name");
                if (cpName) mrhSetLiveVar(cpName, textInput.value);
            });
        ...
        sizeInputs.forEach(function(si) {
            ...
            si.addEventListener("input", function() {
                var siName = si.getAttribute("name");
                if (siName && si.value) {
                    document.documentElement.style.setProperty("--" + siName, si.value);
                }
            });
        });
    }
