<?php
/* -----------------------------------------------------------------------------------------
   $Id: shipping_country_2.php 14588 2022-06-27 06:50:12Z Tomcraft $
...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_shipping_country_2.html', $cache_id) || !$cache) {  
  ...
  if (count($countries_array) > 1 ) {
    $box_content = xtc_draw_form('countries', xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array('action')).'action=shipping_country', $request_type, false), 'post', 'class="box-shipping_country"')
                   . xtc_get_country_list(array('name' => 'country'), (int)$selected, 'class="form-control" onchange="this.form.submit()"')
                   . xtc_hide_session_id();
...
if (!$cache) {
  $box_shipping_country_2 = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_shipping_country_2.html');
} else {
  $box_shipping_country_2 = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_shipping_country_2.html', $cache_id);
}

$smarty->assign('box_SHIPPING_COUNTRY_2', $box_shipping_country_2);
?>