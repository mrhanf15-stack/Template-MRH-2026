<?php
  ...
  if ($_SESSION['cart']->count_contents() > 0) {
	...
	//-- BOF - PRINT FAX ORDER ------------------------
	// set link parameters
	$link_parameters = (defined('TPL_POPUP_CONTENT_LINK_PARAMETERS') ? TPL_POPUP_CONTENT_LINK_PARAMETERS : (defined('POPUP_CONTENT_LINK_PARAMETERS') ? POPUP_CONTENT_LINK_PARAMETERS : '&KeepThis=true&TB_iframe=true&height=400&width=600'));
	
	// set link class
	$link_class = (defined('TPL_POPUP_CONTENT_LINK_CLASS') ? TPL_POPUP_CONTENT_LINK_CLASS : (defined('POPUP_CONTENT_LINK_CLASS') ? POPUP_CONTENT_LINK_CLASS : 'thickbox'));

	// unset javascript popup. force new tab
	$link_parameters = $link_class = '';
		
	// add to smarty
	$box_smarty->assign('BUTTON_FAXORDER', '<a target="_blank" href="'.xtc_href_link(FILENAME_PRINT_FAX_ORDER, $link_parameters, 'SSL').'" title="'.IMAGE_BUTTON_FAX_ORDER.'" class="'.$link_class.'">'.xtc_image_button('button_fax_order.gif', IMAGE_BUTTON_FAX_ORDER).'</a>');
	//-- BOF - PRINT FAX ORDER ------------------------
  }
