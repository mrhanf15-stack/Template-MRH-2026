...
// -----------------------------------------------------------------------------------------
  if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
		...
		require_once(DIR_FS_BOXES . 'shipping_country_2.php');
		}
  }
// -----------------------------------------------------------------------------------------
//	product details
...
// -----------------------------------------------------------------------------------------
  if ($_SESSION['customers_status']['customers_status_show_price'] == '1') {
    if (BS4_STARTPAGE_BOX_ADD_QUICKIE != 'false' || BS4_NOT_STARTPAGE_BOX_ADD_QUICKIE != 'false'){	  
	  ...
	  // get current parent categories id
	  $cat_id = 0;
	  if (!empty($_SESSION['CatPath'])) {
		  $cat_array = explode('_', $_SESSION['CatPath']);
		  $cat_id = (int)$cat_array[0];
	  } else if (!empty($_GET['cPath'])) {
		  $cat_array = explode('_', $_GET['cPath']);
		  $cat_id = (int)$cat_array[0];
	  } else if (!empty($current_category_id)) {
		  $cat_id = (int)$current_category_id;
	  } 
	  
	  // show only in category "Samen Shop" (581210)
	  if (!empty($cat_id) && $cat_id == 581210) {
...
if (defined('MODULE_BLOG_STATUS') 
	...
	if (MODULE_BLOG_BOX_CATEGORIES == 'true') require_once(DIR_FS_BOXES.'blog_categories.php');
	if (MODULE_BLOG_BOX_POSTS == 'true') require_once(DIR_FS_BOXES.'blog_posts.php');
	if (MODULE_BLOG_BOX_CATEGORY == 'true') require_once(DIR_FS_BOXES.'blog_category.php');
}
// -- EOF - BLOG ---------------------------------------------------------------------------
...
// -- BOF - AFFILIATE ---------------------------------------------------------------------------
if (defined('MODULE_AFFILIATE_STATUS') 
	&& MODULE_AFFILIATE_STATUS == 'true'
) {
