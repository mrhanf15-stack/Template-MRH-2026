...
?>
	$('.box_category').on('click', '.category_button', function() {
		...
		if($this.next('ul').length > 0) {
			...
			$.get(DIR_WS_BASE+'ajax.php', {ext: 'bs4_get_subcat', type: 'html', cPath: path, language: '<?php echo $_SESSION['language_code']; ?>', func: 1, speed: 1}, function(data) {
				if (data != '' && data != undefined) {
					$this.after(data);
					$this.next('ul').slideToggle();
					$this.toggleClass("fa-chevron-right fa-chevron-up");
		...
		$('ul#main')
			.on('mouseenter', 'li.hassub', function () {
				...
				if(sel.next('ul').length > 0) {
					...
					$.get(DIR_WS_BASE+'ajax.php', {ext: 'bs4_get_subcat', type: 'html', cPath: path, language: '<?php echo $_SESSION['language_code']; ?>', fpath: fullpath, func: func, speed: 1}, function(data) {
						if (data != '' && data != undefined) {
							sel.after(data);
...
?>
	if($('.canvasmenu').is(':visible')) {
		...
		$('#navbar').attr('class','fixed-top bg-light-top top2 navbar navbar-expand mb-3 text-center clearfix <?php echo BS4_MENUBAR_FIXEDTOP_CLASSES; ?>');
